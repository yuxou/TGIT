function initializePlaceListListeners() {
    ...
    if (!placeList) {
        console.warn("Place list not found. Listener not registered.");
        return;
    }

    console.log("Place list found. Adding event listeners.");

    // 클릭 이벤트 리스너 등록
    placeList.addEventListener("click", (event) => {
        const target = event.target;

        console.log("Clicked element:", target); // 클릭된 요소 디버깅
        ...
        if (target.classList.contains("icon")) {
            ...
            const index = Array.from(placeList.querySelectorAll(".place-item")).indexOf(clickedPlace);
            if (index !== -1) {
                deletePlace(index); // 삭제 함수 호출
            }
        }
    });
}
function showContent(content) {
    if (content === 'makePlan') {
        ...
        initializePlaceListListeners(); // makePlan.html 로드 이후 호출
        ...
        loadHTML('planChecklist.html', 'content-container');
...
document.addEventListener("click", (event) => {
    ...
    if (target.classList.contains("icon")) {
        ...
        const placeList = document.getElementById("place-list");
        if (!placeList) {
            console.error("Place list not found.");
            return;
        }
...
function deletePlace(index) {
    ...
    if (!placeList) {
        console.error("Place list not found during deletePlace.");
        return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // makePlan.html 로드 후 초기화
    loadHTML('makePlan.html', 'content-container', () => {
        console.log("makePlan.html loaded. Checking for required elements...");
        ...
        if (!companionInfoElement || !userListElement) {
            console.warn(
                "Required elements (companion-info or user-list) are missing after makePlan.html load. Initialization skipped."
            );
            return;
        }
